...
function FeaturedRoom({ room, loaded, free }) {
  ...
  const FeaturedWrapper = loaded ? A : DIV;
  return (
    <div>
...
function NextEvent({ nextEvent }) {
  const locations =
    nextEvent && nextEvent.location ? nextEvent.location.split(",") : [];
  ...
  return nextEvent ? (
    <DIV className="NextEvent">
      <p>
        ...
        <strong>{distanceInWordsToNow(nextEvent.start.dateTime) + " "}</strong>(
        <a href={nextEvent.htmlLink}>{nextEvent.summary}</a>)
        {nextEvent.location ? (
          <span>
            {" "}
            in <strong>{linkifiedLocations}</strong>
          </span>
        ) : (
          ""
        )}
...
function selectFeatured(free, busy) {
  ...
  const bestFreeRooms = mainFreeRooms.filter(
    rm => new Date(rm.until) >= fifteenMinFromNow
  );
  const bestBusyRooms = mainBusyRooms.filter(
    rm => new Date(rm.until) <= fifteenMinFromNow
  );
  ...
  if (bestFreeRooms.length) {
    ...
    isFree = true;
  } else if (mainFreeRooms.length) {
    // Then try any other free rooms
    featured = randomChoice(mainFreeRooms);
    isFree = true;
  } else if (bestBusyRooms.length) {
    // then try busy rooms that will be free soon
    featured = randomChoice(bestBusyRooms);
    isFree = false;
  } else if (mainBusyRooms.length) {
    // then try any other busy rooms
...
export default class StatefulApp extends React.Component {
  ...
  componentDidMount() {
    ...
    microfeedback({
      ...
      html:
        '<small>Your feedback will be posted to the <br><a target="_blank" rel="noopener noreferrer" href="https://github.com/sloria/cosroom/issues">sloria/cosroom Issue Tracker</a></small>',
      placeholder: "Report a bug, share an idea, or just say thanks",
      optimistic: false,
      showSuccessDialog(btn, input, response) {
        return btn.alert(
          "Thanks!",
          `Your issue has been posted here:<br>
      ...
      onFailure(btn) {
        return btn.alert(
          "Oops!",
          "Something went wrong…Go tell Steve.",
          "error"
        );
      }
    });
  ...
  componentDidUpdate() {
    ...
    const now = new Date();
    const nextEventNeedsUpdate =
      nextEvent && new Date(nextEvent.start.dateTime) < now;
    const freeRoomsNeedUpdate =
      free.length && free.filter(room => new Date(room.until) < now).length;
    const busyRoomsNeedUpdate =
      busy.length && busy.filter(room => new Date(room.until) < now).length;
    if (
      !error &&
      (nextEventNeedsUpdate || freeRoomsNeedUpdate || busyRoomsNeedUpdate)
    ) {
      this.update();
    }
  }
